...
export default function ProductGrid() {
  ...
  const paginatedProducts = useMemo(() => {
    const sortedProducts = [...products].sort((left, right) => {
      ...
      if (sortOption === 'name-asc') return left.name.localeCompare(right.name, 'vi');
    ...
    <div className="font-sans">
      {showFiltersDrawer && (
        <div className="fixed inset-0 z-50">
          ...
          />
          <aside className="absolute left-0 top-0 h-full w-full max-w-sm bg-white shadow-2xl border-r border-[#EAE3D2]/60 p-5 overflow-y-auto animate-in slide-in-from-left duration-300">
            <div className="flex items-center justify-between gap-3 pb-3 border-b border-gray-100">
              ...
              <button type="button" onClick={() => setShowFiltersDrawer(false)} className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center hover:bg-gray-50">
                <X className="w-4 h-4" />
              </button>
            </div>
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 shrink-0">
              <button 
                ...
                className="flex items-center gap-1.5 px-4 py-2 border border-[#EAE3D2] rounded-full text-xs font-bold bg-white hover:border-[#222222] transition-all cursor-pointer shrink-0 shadow-sm"
              >
              ...
              <div className="relative">
                <select
                  ...
                  className="appearance-none pl-4 pr-10 py-2 border border-[#EAE3D2] rounded-full text-xs font-bold bg-white hover:border-[#222222] transition-all cursor-pointer shadow-sm text-gray-700"
                >
