...
import { Voucher } from '@/features/shared/types';
...
export default function SellerVouchers() {
  ...
  const fetchVouchers = async () => {
    setLoading(true);
    try {
      const data = await sellerService.getVouchers();
      setVouchers(data);
    } catch (err) {
  ...
  const handleToggleActive = async (voucher: Voucher) => {
    ...
    try {
      await sellerService.toggleVoucherActive(voucher.id, nextState);
  ...
  const handleSaveVoucher = async (payload: any) => {
    const created = await sellerService.createVoucher(payload);
                ...
                {filteredVouchers.map((v) => {
                  ...
                  return (
                    <tr key={v.id} className="hover:bg-[#FDFAF7]/30 transition-all">
                      ...
                      <td className="p-4">
                        <h4 className="font-semibold text-gray-850 line-clamp-1 leading-snug">{v.name}</h4>
                      ...
                      <td className="p-4 font-bold text-gray-700">{formatVnd(v.minOrderValue)}</td>
                      <td className="p-4">
                        <div className="space-y-1">
                          <span className="font-bold text-gray-750">{v.usedQuantity} / {v.totalQuantity}</span>
                      ...
                      <td className="p-4 text-center">
                        <button
                          ...
                          className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-[10px] font-bold border transition-colors cursor-pointer ${
                            isActive
                              ? 'bg-[#EBF2EE] text-[#1E5C3F] border-[#1e5c3f]/25 hover:bg-red-50 hover:text-red-650 hover:border-red-200'
