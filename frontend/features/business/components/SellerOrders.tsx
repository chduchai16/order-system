...
import { Order } from '@/features/shared/types';
...
export default function SellerOrders() {
  ...
  const handleConfirmShip = async (e: React.FormEvent) => {
    ...
    try {
      await sellerService.updateOrderStatus(shippingOrderId, 'SHIPPING', carrier, trackingNumber);
  ...
  const handleMarkDelivered = async (orderId: string, amount: number, e: React.MouseEvent) => {
    ...
    try {
      ...
      const currentSettings = sellerService.getShopSettings();
  ...
  const handleCancelOrder = async (orderId: string, e: React.MouseEvent) => {
    ...
    try {
      await sellerService.updateOrderStatus(orderId, 'CANCELLED');
                ...
                {filteredOrders.map((order) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 text-gray-650 truncate max-w-[180px]" title={addressStr}>{addressStr}</td>
                ...
                <div className="space-y-3">
                  ...
                  <div className="text-xs space-y-1 text-gray-650">
                    <p className="flex items-center gap-1.5">
                      ...
                      <span className="font-bold text-gray-800">Mã khách hàng: #{selectedOrder.userId}</span>
                    </p>
            ...
            <div className="px-6 py-4.5 border-t border-[#EAE3D2]/50 flex justify-end gap-3 bg-[#FDFAF7]">
              ...
              {(selectedOrder.status === 'PENDING' || selectedOrder.status === 'PAID') && (
                <button
                  ...
                  className="px-5 py-2 border border-red-200 hover:bg-red-50 text-red-650 rounded-full text-xs font-bold transition-all cursor-pointer"
                >
                  Huỷ đơn hàng
                </button>
              )}
