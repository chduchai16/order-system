...
import { Product, Order } from '@/components/types';
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const ordList = await orderService.getSellerOrders();
        setOrders(ordList);
      } catch (err) {
    ...
    <div className="space-y-6">
      ...
      {settings && (
        <div className="bg-[#0e4a42] text-white rounded-3xl p-6 md:p-8 flex flex-col md:flex-row justify-between items-center gap-6 shadow-md relative overflow-hidden group">
          ...
          <div className="flex flex-wrap gap-3 shrink-0 relative z-10 w-full md:w-auto">
            ...
            <Link href="/seller/orders" className="flex-1 md:flex-initial inline-flex items-center justify-center gap-1.5 bg-white/10 hover:bg-white/15 border border-white/20 text-white font-bold text-xs px-5 py-3 rounded-full transition-all">
              Xử lý đơn hàng
        ...
        <div className="space-y-6">
              ...
              {lowStockProducts.map(p => (
                <div key={p.id} className="flex items-center justify-between p-2.5 bg-[#FDFAF7] border border-[#EAE3D2]/40 rounded-xl">
                  ...
                  <div className="text-right shrink-0 ml-3">
                    ...
                    <Link href="/seller/products" className="text-[9.5px] text-[#F1641E] font-bold block hover:underline mt-0.5">
                      Cập nhật +
                    </Link>
                  </div>
                </div>
          ...
          <div className="bg-white border border-[#EAE3D2]/50 p-5 rounded-3xl shadow-sm text-left">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <Link href="/seller/products" className="flex flex-col items-center justify-center p-3 border border-gray-150 hover:border-[#F1641E]/40 hover:bg-[#FDFAF7] rounded-2xl transition-all text-center group cursor-pointer">
      ...
      <div className="bg-white border border-[#EAE3D2]/50 p-6 rounded-3xl shadow-sm text-left">
        <div className="flex justify-between items-center mb-5 border-b border-[#EAE3D2]/40 pb-3">
          ...
          <Link href="/seller/orders" className="text-xs font-bold text-[#F1641E] hover:underline flex items-center gap-1">
            Quản lý đơn hàng
